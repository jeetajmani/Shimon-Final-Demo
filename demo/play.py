...
from cv import eye_contact_enabled
import threading
import numpy as np
import mido
import copy
...
looping_active = threading.Event()
loop_stop = threading.Event()

loop_temperature = 0.0
loop_temp_lock = threading.Lock()

loop_bpm = 120.0
loop_bpm_lock = threading.Lock()
...
def process_loop_phrase(events, temperature):
    """Seu-style variation: randomly swap notes with others from the phrase.
    Temperature controls how many notes change (0 = none, 1 = up to all)."""
    temperature = float(np.clip(temperature, 0, 1))
    active_indices = [i for i, e in enumerate(events) if e["velocity"] > 0]
    n_active = len(active_indices)
    if n_active < 2:
    ...
    end = int(n_active * temperature)
    if end <= 0:
        return events
    n_to_change = int(np.random.randint(0, end + 1))
    if n_to_change <= 0:
        return events

    w = np.hanning(n_active) + 1e-6
    p = w / np.sum(w)
    indices = np.random.choice(n_active, size=min(n_to_change, n_active), replace=False, p=p)

    options = list(set(events[i]["note"] for i in active_indices))
    if not options:
        return events

    for pos in indices:
        event_idx = active_indices[pos]
        events[event_idx]["note"] = int(np.random.choice(options))

    return events


def _nod_to_tempo(bpm_unused, stop_event):
    """Nod Shimon's head on each beat until stop_event is set.
    Reads loop_bpm each beat for live tempo changes."""
    direction = 1
    while not stop_event.is_set():
        with loop_bpm_lock:
            beat_interval = 60.0 / loop_bpm
        neck_pos = 0.05 * direction
        head_pos = -0.2 * direction
        send_gesture_to_shimon("NECK",     neck_pos, 15)
        send_gesture_to_shimon("HEADTILT", head_pos, 20)
        direction *= -1
        stop_event.wait(beat_interval)


def play_loop_sequence(events_original, base_bpm, stop_event):
    """Play the phrase in a loop with variations until stop_event is set.
    Notes are sent to Max via /note. Head nodding runs on a separate
    tempo-synced thread. Temperature and BPM are read live each loop."""
    # Start nodding thread
    nod_stop = threading.Event()
    nod_thread = threading.Thread(
        target=_nod_to_tempo, args=(base_bpm, nod_stop), daemon=True
    )
    nod_thread.start()
    ...
    try:
        while not stop_event.is_set():
            loop_num += 1
            with loop_temp_lock:
                current_temp = loop_temperature
            with loop_bpm_lock:
                current_bpm = loop_bpm
            tempo_scale = base_bpm / current_bpm
            events = copy.deepcopy(events_original)
            events = process_loop_phrase(events, current_temp)
            if not events:
                break
            events[0]["delta"] = 0.01
            ...
            for event in events:
                if stop_event.is_set():
                    break
                time.sleep(event["delta"] * tempo_scale)
                if event["velocity"] != 0:
                    send_to_max("/note", event["note"])

            print(f"\U0001f501 Loop {loop_num} complete")
    finally:
        nod_stop.set()
        nod_thread.join(timeout=1.0)


def loop_phrase(port_index, bpm):
    """Record a phrase, wait for eye contact, then loop until thumbs up."""
    global part_1_flag
    part_1_flag = 2  # eye contact mode
    loop_stop.clear()
    recording_done.clear()
    ...
    print("\U0001f3b9 Play a phrase... (look at Shimon when ready)")

    # Record until eye contact — only start accepting after first note
    while True:
        ...
        if msg:
            ...
            print(event)
            i += 1
            if not accepting_eye_contact.is_set():
                accepting_eye_contact.set()
        else:
            time.sleep(0.001)

        # Eye contact arrived — stop recording immediately
        if recording_done.is_set() and len(events) >= 1:
            print(f"\U0001f4dd Phrase captured ({len(events)} events) — eye contact")
            break

    # If eye contact hasn't happened yet, wait for it
    if not recording_done.is_set():
        print("\U0001f440 Look at Shimon to start looping!")
        recording_done.wait()

    # Start looping (runs forever until Ctrl+C)
    print("\U0001f501 Starting loop")
    look_forward()
    looping_active.set()
    eye_contact_enabled.clear()  # disable eye contact, hand gestures still active
    ...
    with loop_bpm_lock:
        loop_bpm = bpm
    play_loop_sequence(events_original, bpm, loop_stop)

    looping_active.clear()
    eye_contact_enabled.set()
    print("\u23f9 Loop stopped")
    look_left()


def on_turn_take(source="eye_contact"):
    if looping_active.is_set():
        return
...
def on_gesture(param, direction):
    global loop_temperature, loop_bpm
    if param == "STOP" and looping_active.is_set():
        print("✋ Palm detected — stopping loop")
        loop_stop.set()
    elif param == "TEMP" and looping_active.is_set():
        step = 0.1 if direction == "+" else -0.1
        with loop_temp_lock:
            loop_temperature = float(np.clip(loop_temperature + step, 0.0, 1.0))
            current = loop_temperature
        print(f"🌡 Temperature: {current:.1f}")
    elif param == "TEMPO" and looping_active.is_set():
        step = 2 if direction == "+" else -2
        with loop_bpm_lock:
            loop_bpm = float(np.clip(loop_bpm + step, 40, 300))
            current = loop_bpm
        send_to_max("/tempo", current)
        print(f"🎵 Tempo: {current:.0f} BPM")
...
def tempo_detect():
    ...
    print(f"Tempo confirmed: {bpm:.1f} BPM")
...
if __name__ == "__main__":
    ...
    start_gestures_monitor(on_turn_take, on_tempo_detected, on_gesture)
    look_left()
    for i in range(5):
        keyboard_phrase(port_index)
    
    print("START TEMPO PART")
    idle(0)
    detected_bpm = tempo_detect()

    print(f"\n\U0001f501 LOOP MODE \u2014 BPM: {detected_bpm}")
    print("Play a phrase \u2192 look at Shimon \u2192 loop starts \u2192 thumbs up to stop")

    try:
        while True:
            loop_phrase(port_index, detected_bpm)
    except KeyboardInterrupt:
