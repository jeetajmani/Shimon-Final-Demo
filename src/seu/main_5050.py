...
class Shimon5050:
    ...
    def _ignore_handler(self, address, *args):
        print(f"⚠️ Gesture Ignored: {address} is automated in 50/50 mode.")
    ...
    def run(self):
        ...
        hud_image = np.zeros((320, 450, 3), dtype=np.uint8); window_name = "Shimon - 5050"
        while self.running:
            hud_image.fill(0) 
            cv2.putText(hud_image, f"Mode: 50/50 INTERMEDIATE", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
...
if __name__ == '__main__':
    ...
    qna_params = { "raga_map": [1, 0, 1, 1, 0, 1, 0, 1, 0, 1, 1, 0], "instruments": (Instrument("Violin", 1), Instrument("Keys", 1, False)), "input_dev_name": "Scarlett 2i2 USB", "randomness_temperature": 0.5, "auto_random_temp": True, "auto_random_loop": False, "temp_min": 0.1, "temp_max": 1.0, "timeout_sec": 0.8 }
    
    demo = Shimon5050('Portable Grand Port 1', False, 98, qna_params, { "smoothing": 5, "n_beats_to_track": 8, "timeout_sec": 2, "tempo_range": (50, 120) }, { "midi_files": [["phrases/intro.mid"]], "gesture_midi_files": [["gestures/intro.mid"]], "note_mapping": [96, 98, 100] }, performer_params)
    demo.qna_demo.repeat_count = 1 
    demo.run()