...
class ShimonRobot:
    def __init__(self, keyboard_name, has_note_off_support, mode_key, qna_param, bd_param, song_param, performer_param):
        ...
        # Temperature & Loop Gestures Blocked
        self.osc_dispatcher.map("/shimon/temperature/add", self._ignore_handler)
        self.osc_dispatcher.map("/shimon/loop/add", self._ignore_handler)
    ...
    def _ignore_handler(self, address, *args):
        print(f"⚠️ Gesture Ignored: {address} is automated in ROBOT mode.")
    ...
    def run(self):
        ...
        hud_image = np.zeros((320, 450, 3), dtype=np.uint8); window_name = "Shimon - ROBOT"
        while self.running:
            hud_image.fill(0) 
            cv2.putText(hud_image, f"Mode: ROBOT-DOMINANT", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
            cv2.putText(hud_image, f"Temperature: {self.qna_demo.randomness_temperature:.2f} (Auto)", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.putText(hud_image, f"Loop Count: {self.qna_demo.repeat_count} (Auto)", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
...
if __name__ == '__main__':
    gesture_note_mapping = { "beatOnce": 50, "breath": 51, "look": 52, "circle": 53, "nodsway": 54, "ar_sway": 55, "eyebrows": 56, "headcircle": 57, "cooldown": 58, "scream": 59, "headcirclefast": 60, "no": 61 }
    performer_params = { "osc_address": "127.0.0.1", "osc_port": 20000, "gesture_note_mapping": gesture_note_mapping, "osc_arm_route": "/arm", "osc_head_route": "/head", "min_note_dist_ms": 100, "max_notes_per_onset": 3, "virtual": False }
    qna_params = { "raga_map": [1, 0, 1, 1, 0, 1, 0, 1, 0, 1, 1, 0], "instruments": (Instrument("Violin", 1), Instrument("Keys", 1, False)), "input_dev_name": "Scarlett 2i2 USB", "randomness_temperature": 0.75, "auto_random_temp": True, "auto_random_loop": True, "temp_min": 0.5, "temp_max": 1.0, "timeout_sec": 0.8 }
    
    demo = ShimonRobot('Portable Grand Port 1', False, 98, qna_params, { "smoothing": 5, "n_beats_to_track": 8, "timeout_sec": 2, "tempo_range": (50, 120) }, { "midi_files": [["phrases/intro.mid"]], "gesture_midi_files": [["gestures/intro.mid"]], "note_mapping": [96, 98, 100] }, performer_params)
    demo.run()